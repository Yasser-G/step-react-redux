const stateSetter = (state: object, key: string, payload: object) => {
    ...
    const logSuccess = () => console.log(`StepReactRedux.${key}`, payload);
    ...
    switch (depth) {
        ...
        default:
            console.warn(`StepReactRedux Failed to change deep state: ${key}./nMake Sure that its parent values are initialized before changing its children values.`);
            return state;
    }
